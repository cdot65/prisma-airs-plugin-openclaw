...
if [ -n "${PANW_AI_SEC_API_KEY:-}" ]; then
  echo "[entrypoint-npm] Setting Prisma AIRS config from environment..."
  node -e "
    const fs = require('fs');
...
if [ -n "${LITELLM_API_KEY:-}" ]; then
  echo "[entrypoint-npm] Setting LiteLLM API key in models config..."
  node -e "
    const fs = require('fs');
    const cfg = JSON.parse(fs.readFileSync(process.env.LIVE_CONFIG, 'utf8'));
...
if [ -n "${OPENAI_API_KEY:-}" ]; then
  echo "[entrypoint-npm] Setting OpenAI API key in models config..."
  node -e "
    const fs = require('fs');
    const cfg = JSON.parse(fs.readFileSync(process.env.LIVE_CONFIG, 'utf8'));
